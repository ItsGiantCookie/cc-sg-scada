local SGMSI_VERSION = "v0.1-ALPHA"

local install_dir = "/.install-cache"
local manifest_path = "https://ItsGiantCookie.github.io/cc-sg-scada/manifests/"
local repo_path = "http://raw.githubusercontent.com/ItsGiantCookie/cc-sg-scada/"
...
if _is_pkt_env then println("- SCADA Installer "..SGMSI_VERSION.." -")
else println("-- CC Mekanism SCADA Installer "..SGMSI_VERSION.." --") end

if #opts == 0 or opts[1] == "help" then
    println("usage: sgmsi <mode> <app> <branch>")
    if _is_pkt_env then
    ...
    local next_opt = 3
    local apps = { "gate-plc", "supervisor", "coordinator", "installer" }
...
if mode == "check" then
    ...
    if not local_ok then
        ...
        local_manifest = { versions = { installer = SGMSI_VERSION } }
    else
        local_manifest.versions.installer = SGMSI_VERSION
    end
    ...
    if manifest.versions.installer ~= local_manifest.versions.installer and not _is_pkt_env then
        yellow();println("\nA different version of the installer is available, it is recommended to update (use 'sgmsi update installer').");white()
    end
elseif mode == "install" or mode == "update" then
    ...
    if r_manifest.versions.installer ~= SGMSI_VERSION then
        ...
        if update_installer or ask_y_n("Would you like to update now", true) then
            lgray();println("GET sgmsi.lua")
            local dl, err = http.get(repo_path.."sgmsi.lua")
